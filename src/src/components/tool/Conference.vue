  ...
  export default {
    ...
    methods: {
      init () {
        ...
        connection.onstream = (event) => {
          ...
          if (event.type === 'local') {
            ...
            connection.openOrJoin(String(this.loginInfo.phonenum), (isRoomOpened, roomid, error) => {
              if (error) {
        ...
        // connection.openOrJoin(String(this.loginInfo.phonenum))
      },
      goLogin () {
        this.$router.replace({
          name: 'login'
      ...
      setRoomId () {
        ...
        if (this.roomId.match(/^@/)) {
          // call user
          let calleeUserName = this.roomId.replace(/^@/, '')
          this.connection.checkPresence(calleeUserName, (isOnline, username) => {
            if (!isOnline) {
              this.$Notice.info({
                desc: calleeUserName + '不在线'
              })
              return
            }

            this.connection.join(String(this.loginInfo.phonenum), (isRoomJoined, roomid, err) => {
              if (err) {
