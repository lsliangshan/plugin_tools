<template>
  <div class="id_photo_container"
        ...
        <div class="bottom_container">
          ...
          <Tooltip placement="top"
            ...
            <div slot="content">
              ...
              <div v-else>生成证件照，点击设置按钮设置不同背景色</div>
            </div>
            <Button type="primary"
                    class="capture_btn"
                    icon="md-camera"
    ...
    <Drawer title="证件照"
      ...
      <transition name="id-image-blank-transition"
                  enter-active-class="animated fadeIn"
                  leave-active-class="animated fadeOut">
        <div class="id_image_wrapper_blank"
             v-if="!idImageForm.imageData && !idImageForm.isFetch">
          <Button type="primary"
                  class="capture_btn"
                  icon="md-camera"
                  :disabled="!isLogin"
                  @click="getIdImage">重新生成</Button>
        </div>
      </transition>
