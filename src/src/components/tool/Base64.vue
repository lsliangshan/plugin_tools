        ...
        <Split v-model="split"
          ...
          <div slot="top"
            ...
            <Input type="textarea"
                   ...
                   :value="unencodedText || decodedText"
                   placeholder="解码结果或待编码的字符（正常字符）"
                   @on-change="changeUnencodedText"
                   :class="{'error_input': decodedText === defaultErrorText}" />
          </div>
          <div slot="trigger"
            ...
            <p class="tips_right">Base64格式的字符
          ...
          <div slot="bottom"
            ...
            <Input type="textarea"
                   autofocus
                   class="unformat_wrapper no_bg"
                   :value="undecodedText || encodedText"
                   placeholder="编码结果或待解码的字符（Base64格式的字符）"
                   @on-change="changeUndecodedText" />
          </div>
        </Split>
      </TabPane>
      <TabPane label="文件编码">
        <Split v-model="split2"
            ...
            <div class="decode_image_wrapper">
              <div class="decode_image_preview">
                <img :src="decodedImage"
                     alt="文件预览"
                     v-if="imageTypes.indexOf(fileType) > -1">
                <p v-else
                   class="preview_other_type">
          ...
          <div slot="bottom"
            ...
            <Input type="textarea"
                   ...
                   class="unformat_wrapper no_bg"
                   :value="decodedImage"
                   placeholder="编码结果或待解码的字符"
                   @on-change="changeImageBase64" />
          </div>
        </Split>
      </TabPane>
    </Tabs>
  ...
  methods: {
    getQueryStr () {
      if (this.$route.query.q) {
        this.unencodedText = decodeURIComponent(this.$route.query.q.trim())
        this.changeUnencodedText({
          target: {
            value: this.unencodedText
          }
        })
      }
    },
